// Modified by: Zhou Fang <fang2hou@outlook.com>
...
func makeAppBundle(appFilename string) error {
	...
	infoPlist := strings.ReplaceAll(infoPlistTpl, "{{.AppName}}", binaryName)
	infoPlist = strings.ReplaceAll(infoPlist, "{{.BundleIdentifier}}", bundleIdentifier)
	infoPlistPath := filepath.Join(appFilename, "Contents", "Info.plist")
	err := os.WriteFile(infoPlistPath, []byte(infoPlist), 0600)
